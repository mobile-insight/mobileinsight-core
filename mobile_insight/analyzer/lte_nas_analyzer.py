...
class LteNasAnalyzer(ProtocolAnalyzer):
    ...
    def __callback_esm_state(self, msg):
        ...
        self.__esm_status[self.__cur_eps_id].eps_id = int(
            msg.data["EPS bearer ID"])
        self.__esm_status[self.__cur_eps_id].type = int(
            msg.data["EPS bearer type"])
    ...
    def __callback_esm(self, msg):
        ...
        for field in msg.data.iter('field'):
            ...
            if field.get('name') == "nas_eps.emm.qci":
                self.__esm_status[self.__cur_eps_id].qos.qci = int(
                    field.get('show'))
    ...
    def get_profiled_qos(self, plmn):
        ...
        if plmn:
            ...
            def f_qos_int(x): return int(x) if x and x != "unknown" else None

            def f_qos_float(x): return float(
                x) if x and x != "unknown" else None
            res = EsmQos()
            ...
            res.traffic_handling_priority = f_qos_int(
                tmp['traffic_handling_priority'])
            res.max_bitrate_ulink = f_qos_int(tmp['max_bitrate_ulink'])
            res.max_bitrate_dlink = f_qos_int(tmp['max_bitrate_dlink'])
            res.guaranteed_bitrate_ulink = f_qos_int(
                tmp['guaranteed_bitrate_ulink'])
            res.guaranteed_bitrate_dlink = f_qos_int(
                tmp['guaranteed_bitrate_dlink'])
            ...
            res.guaranteed_bitrate_ulink_ext = f_qos_int(
                tmp['guaranteed_bitrate_ulink_ext'])
            res.guaranteed_bitrate_dlink_ext = f_qos_int(
                tmp['guaranteed_bitrate_dlink_ext'])
...
class EsmQos:
    ...
    def dump_rate(self):
        ...
        return (self.__class__.__name__ +
                ...
                ' max_bitrate_ulink_ext=' +
                xstr(self.max_bitrate_ulink_ext) +
                ' max_bitrate_dlink_ext=' +
                xstr(self.max_bitrate_dlink_ext) +
                ' guaranteed_birate_ulink_ext=' +
                xstr(self.guaranteed_bitrate_ulink_ext) +
