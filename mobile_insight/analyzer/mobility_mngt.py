        ...
        for field in msg.data.iter('field'):
            ...
            if field.get('name') == "lte-rrc.mobilityControlInfo_element":
                ...
                if target_cell:
                    ...
                    self.__mobility_state_machine.update_state_machine(
                        self.__handoff_sample)
                    # Reset handoff sample
                    self.__handoff_sample = HandoffSample()

                    self.log_info(str(string2timestamp(msg.timestamp)) +
            ...
            if field.get(
                    'name') == "lte-rrc.handoverFromEUTRAPreparationRequest_element":
            ...
            if field.get('name') == "lte-rrc.redirectedCarrierInfo":
                ...
                for val in field.iter('field'):
                    ...
                    if val.get('name') == 'lte-rrc.utra_FDD':
                        ...
                        self.__handoff_sample.add_state_transition(
                            handoff_state)
                        # Trigger merging function
                        self.__mobility_state_machine.update_state_machine(
                            self.__handoff_sample)
                        ...
                        self.log_info(str(string2timestamp(
                            msg.timestamp)) + " Redirect to " + handoff_state.dump())
            ...
            if field.get('name') == "lte-rrc.measurementReport_element":
                ...
                if meas_id and self.__handoff_sample.cur_state:
                    ...
                    if meas_report[1]:  # report config is known
                        ...
                        bcast_dict = {}
                        bcast_dict['Timestamp'] = str(msg.timestamp)
                        bcast_dict['event'] = str(
                            meas_report[1].event_list[0].type)
            ...
            if field.get('name') == "lte-rrc.measResultsCDMA2000_element":
                ...
                # NOTE: Different from normal meas report, this one does not
                # have measid/reportid
                tmp = LteReportConfig("CDMA2000", 0)
                tmp.add_event("CDMA2000", 0)
                meas_report = (
                    LteMeasObjectCDMA2000(
                        None, 0), tmp)  # fake an empty report

                self.__handoff_sample.add_meas_report(meas_report)
                self.log_info(str(string2timestamp(msg.timestamp)) +
                              " Measurement report cdma2000 " + str(rss))
