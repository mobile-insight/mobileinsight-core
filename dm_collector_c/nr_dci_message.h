...
const Fmt NrDciMessage_RawDci_Fmt[] = {
        {UINT, "Raw DCI[0]", 4}, // 32 bits
        {UINT, "Raw DCI[1]", 4}, // 32 bits
        {UINT, "Raw DCI[2]", 4} // 32 bits
}
...
const Fmt NrDciMessage_UL_0_1_Fmt[] = {
        ...
        {UINT, "Antenna Ports", 2}, // ? bits, 2 byte = 16 bits cover Antenna Ports, DMRS Seq Init Flag, SRS Request, CSI Request, CBGTI, Reserved MCE Enable, RA Type. Do bit masking on the 2 bytes to get Antenna Ports
...
static int _decode_nr_DCI (const char *b,
    ...
    for (unsigned int recordIndex = 0; recordIndex < numRecords; recordIndex++) {
        ...
        /*
        28 02 = 0010 1000 0000 0010 - note this is in little endian!
        Frame = 552 = 28 x2  = 0010 1000 xxxx 0010
        Reserved = 0 = xx 02 = xxxx xxxx 0000 0010

        552 in hexadecimal is 0x228 = 1000 1010 0000

        So x = (Frame & 0xFF03) = 0x2802 - to get 552, we need 0x0228
        We get the leftmost 4 bits and shift it 8 bits to the left: y = (x & 0x000F) << 8 = 0x0200
        We shift x right 8 bits: z = x >> 8 = 0x0028
        Now we do ans = y | z = 0x0200 | 0x0028 = 0x0228 = 552!
        */
        utemp = _search_result_uint(record, "Frame");
        unsigned int x = (utemp & 0xFF03);
        unsigned int y = (x & 0x000F) << 8;
        unsigned int z = x >> 8;
        unsigned int frame = y | z;
