...
class HandoffLoopAnalyzer(Analyzer):
	...
	def __loop_detection(self,msg):
		...
		for cell in cell_list:
			...
			neibhbor_cells=[] #all neibghboring cells (including cells under same freq)
			for freq in inter_freq_dict:
				neibhbor_cells+=self.__rrc_analyzer.get_cell_on_freq(freq)
			#initially all links are marked unvisited
			cell_config_visited[cell]={x:False for x in neibhbor_cells}
